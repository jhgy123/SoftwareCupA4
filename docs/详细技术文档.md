...
### 3.1 目标提取服务：
智云盎提供一键式目标提取服务，用户登录页面后进入该功能页面进行原始遥感图像上传，借助百度飞桨平台提供的模型将图像中的“操场”这一物体作为目标进行快速提取，提取结果具有高准确率特点。
  * 目标提取功能进入页面如下：
...
### 3.2 目标检测服务：
智云盎目标检测服务为“道路”为要素，用户一键式上传待检测图像即可得到反馈结果。道路信息不仅是基础的地理信息，且为提取其他地物提供了参考，在应急响应、智慧城市、车辆管理、无人机导航等多个领域发挥重要作用。
  * 目标检测功能进入页面如下：
